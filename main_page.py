...
def next_page():
    root.destroy()
    os.system("py main_page2.py")



root.geometry('1500x1500')
img = ImageTk.PhotoImage(Image.open("project_images/volley.jpg").resize((400,200), Image.ANTIALIAS))
image = Label(root, image = img).grid(row = 0, column = 0, sticky = W, pady = 20,padx=220)
Label_2=Label(root, text="Volley Ball",width=20,font=("bold", 15)).place(x=400,y=10)
...
img_2 = ImageTk.PhotoImage(Image.open("project_images/shuttle.jpg").resize((400,200), Image.ANTIALIAS))
image_2 = Label(root, image = img_2).grid(row = 5, column = 0, sticky = W, pady = 20,padx=220)
Label_3=Label(root, text="Badminton",width=20,font=("bold", 15)).place(x=400,y=320)
